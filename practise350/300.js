...
function createEpisodeCard(episode) {
  ...
  seasonEpisode.textContent = `S${episode.season.toString().padStart(2, '0')}E${episode.number.toString().padStart(2, '0')}`;
...
function handleEpisodeSelect(event) {
  ...
  if (selectedEpisodeId) {
    filteredEpisodes = episodes.filter((episode) => episode.id === parseInt(selectedEpisodeId));
  } else {
...
document.getElementById('episode-select').addEventListener('change', handleEpisodeSelect);
document.getElementById('show-select').addEventListener('change', handleShowSelect);
