...
function makePageForEpisodes(episodeList) {
  ...
  episodeList.forEach((episode) => {
    ...
    // add the season and episode and name
    const paragraph = document.createElement("h3");
    paragraph.textContent = `${episode.name} - ${makeSeasonAndEpisode(
      episode
    )}`;
    item.appendChild(paragraph);
...
searchInput.addEventListener("input", async (e) => {
  ...
  if (select === "all-shows") {
    ...
    const filteredEpisodes = shows.filter((episode) => {
      ...
      return episode.name.toLowerCase().includes(searchTerms);
    });
...
dropEpisodes.addEventListener("change", async (e) => {
  ...
  const filteredEpisodes = episodes.filter((episode) => {
    // localeCompare might be neater here
    return select.includes(episode.id);
  });
...
function draw() {
  ...
  for (var k = 0; k < amount; k++) {
    ...
    for (var i = 0; i < width + detail; i += detail) {
      ...
      y +=
        sin(
          i * 0.04 - _o + 10 + offset + noise(_o * 0.1 + (i / width) * 5) * 10
        ) * ampl;
      vertex(i, y);
    }
